...
def open_file():
    """Returns a list with all possible words"""
...
def random_word_picker(moeilijkheidsgraad):
    """Chooses a random 7 letter word to create the game
       Takes one argument as input that decides the difficulty"""
    if moeilijkheidsgraad == 'willekeurig':
        moeilijkheden = [
            'makkelijk', 'gemiddeld', 'moeilijk', 'extreem_moeilijk']
...
def charCount(word):
    """function that is needed for the function all possible words,
        It is used to compare words with the 7 letter word"""
    dict = {}
    for i in word:
...
def possible_words(input, random_list):
    """Compares words with the random 7 letter word,
       to see if they are possible to make.
    ...
    final_word_list = [
        word for word in list_valid_words if middle_char.findall(word)]
    return final_word_list


def save_input():
    """This function makes the Graphic User Interface"""
    ...
    if woordinput in list_valid_words1:
        ...
        label2.configure(
            width=10, background='lightgray', font=("Courier", 30))
...
def give_hint():
    """This function returns a hint that is a word that still needs
       to be quessed"""
    global score
    score -= 1
    ...
    if len(hint_word) == 4 or len(hint_word) == 5:
        ...
        return "Er is een woord over met deze vorm:  _ {0} _ {1} _".format(
            reveal_2nd_letter, reveal_4th_letter)
    elif len(hint_word) == 6:
        reveal_1st_letter = hint_word[0]
        reveal_3rd_letter = hint_word[2]
        reveal_5th_letter = hint_word[4]
        return "Er is een woord over met deze vorm: {0} _ {1} _ {2} _".format(
            reveal_1st_letter, reveal_3rd_letter, reveal_5th_letter)
    else:
        reveal_1st_letter = hint_word[0]
        reveal_2nd_letter = hint_word[1]
        reveal_4th_letter = hint_word[3]
        ...
        return (
            "Er is een woord over met deze vorm: {0}{1} _ {2} _ _ {3}..."
            .format(reveal_1st_letter, reveal_2nd_letter,
                    reveal_4th_letter, reveal_7th_letter))


def hint_venster():
    """Pops a venster with a hint"""
    hint_popup = tk.Tk()
...
def error_check(argv):
    if len(argv) == 3:
        ...
        print(
            'Gebruik: {} *moeilijkheidsgraad (willekeurig, makkelijk, '
            'gemiddeld, moeilijk , extreem_moeilijk)* *toon antwoorden (BOOL)*'
...
if __name__ == '__main__':
    ...
    label1 = Label(
        root, text='Zoek woorden van vier of meer letters in de honingraad.')
    ...
    button4 = Button(
        root, text=random_list[3], command=klik4,
        highlightbackground='#FEE12B', bg='#FEE12B')
    ...
    clearbutton = Button(
        root, text="Clear", command=cleartext, height=1, width=10,
        highlightbackground='green', bg='green')
    submitbutton = Button(
        root, text='Submit', height=1, width=10, command=save_input,
        highlightbackground='green', bg='green')
    hintbutton = Button(
        root, text='Hint?', height=1, width=10, command=hint_venster,
        highlightbackground='#FEE12B', bg='#FEE12B')
